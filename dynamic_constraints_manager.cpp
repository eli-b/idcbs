...
bool DynamicConstraintsManager::isDynCons(int curr_id, int next_id, int next_ts) {
	...
	/* Old: we transform all vertex constraints to edge constraints.
	// Check for vertex constraint (being in next_id at next_timestep is disallowed).
	if ( next_ts < static_cast<int>(dyn_constraints_.size()) ) {
